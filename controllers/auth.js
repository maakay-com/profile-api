    ...
    try {
        ...
        if (!(account_number && signature)) {
            return res.json({
                ...
                "message": "User validation failed: account_number: Path `account_number` and `signature` is required."
            });
        }
        ...
        if (!user) {
            return res.json({
                ...
                "_message": "User validation failed",
                "name": "ValidationError",
                "message": "User validation failed: User associated with `account_number` does not exists."
            })
        }
